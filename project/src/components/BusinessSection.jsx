  ...
  useEffect(() => {
    if (domains?.length > 0 && selectedWebsiteId) {
      ...
      if (typeof selectedWebsiteData?.colors === "string") {
        processedColors = selectedWebsiteData.colors
          .split(",")
          .map((color) => color.trim())
          .filter((color) => color !== "");
      } else if (Array.isArray(selectedWebsiteData?.colors)) {
        processedColors = selectedWebsiteData.colors.filter(
          (color) => color !== ""
        );
      }
    ...
    const handleFileUpload = async () => {
      ...
      if (typeof selectedDomain?.colors === "string") {
        originalColors = selectedDomain.colors
          .split(",")
          .map((color) => color.trim())
          .filter((color) => color !== "");
      } else if (Array.isArray(selectedDomain?.colors)) {
        originalColors = selectedDomain.colors.filter((color) => color !== "");
      }
