...
import toast from "react-hot-toast";
  ...
  console.log(payload)
      const userData = await googleAuth(payload);
      localStorage.setItem("user", JSON.stringify(userData.user));
      localStorage.setItem("token", JSON.stringify(userData.token));
      toast.success("Google sign-in successful!");
      setIsSignInPopup(false);
      ...
      setLoading(false); // Re-enable the button once done
    }
  };
    ...
    window.google.accounts.id.initialize({
      client_id: "334445684697-p0hj5muaed53rquvet0svco0q5loo7c8.apps.googleusercontent.com", // Replace with your Google OAuth Client ID
      callback: handleGoogleLogin, // Handle the response once the user logs in
    });
    window.google.accounts.id.prompt(); // Prompt the user for Google login
  };
    return (
